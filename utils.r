...
theme_border <- function(type = c("left", "right", "bottom", "top", "none"), colour = "black", size = 1, linetype = 1) {
  # use with e.g.: ggplot(...) + theme( panel.border=theme_border(type=c("bottom","left")) ) + ...
...
# ggplot(data=df, aes(x=x, y=y)) + geom_point() + theme_bw() + theme( panel.border = theme_border_numerictype(9) ) 
...
DecoratePlot <- function(gplot) {
    ...
    gplot <- gplot + theme(panel.grid.major = theme_blank())
    gplot <- gplot + theme(panel.grid.minor = theme_blank())
    gplot <- gplot + theme(panel.border = theme_L_border())
    gplot <- gplot + theme(plot.margin = unit(c(0.2,0.4,0,0), "lines"))  # delete axis titles: c(0,0,-1,-1)
...
PlotCumulative <- function(df_) {
    ...
    g <- g + theme(legend.position = c(0.5,0.86),
